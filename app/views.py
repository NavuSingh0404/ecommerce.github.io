...
from .forms import CustomerRegistreationForm,MyPasswordResetForm,CustomerProfileForm
...
class ProductView(View):
    def get(self,request):
        ...
        return render (request,'app/home.html',{'topwears':topwears,'bottomwear':bottomwear,'mobile':mobile,'laptop':laptop,'totalitem':totalitem})
...
def show_cart(request):
    ...
    if request.user.is_authenticated:
        ...
        total_amount = 0.0
        cart_product = [p for p in Cart.objects.all() if p.user == user]   #### least comprehension
        # print(cart_product)
        if cart_product:
...
def mobile(request,data=None):
    ...
    elif data == 'Redmi' or data == 'Samsung' or data == 'Iphone' or data == 'OnePlus' or data=='Asus': 
...
def laptops(request,data=None):
    ...
    elif data == 'Apple' or data == 'Acer' or data == 'MSI' or data == 'DELL' or data=='HP': 
...
def topwears(request,data=None):
    ...
    if data == None:
        topwears = Product.objects.filter(category='TW')
    elif data == 'Theory' or data == 'Cloudtail' or data == 'Texco' or data == 'Zoire': 
...
def bottomwears(request,data=None):
    ...
    elif data == 'Jeanex' or data == 'Levis' or data =='Outer' or data == 'Tavasya' or data =='Fashions': 
...
class CustomerRegistreationView(View):
    def get(self,request):
        form = CustomerRegistreationForm()
        totalitem=0
        if request.user.is_authenticated:
            totalitem = len(Cart.objects.filter(user=request.user))
        return render(request,'app/customerregistration.html',{'form':form,'totalitem':totalitem})
    def post(self,request):        
        form = CustomerRegistreationForm(request.POST)
        if form.is_valid():
            form.save()
...
def checkout(request):
    ...
    if request.user.is_authenticated:
        totalitem = len(Cart.objects.filter(user=request.user))
    address = [p for p in Customer.objects.all() if p.user == request.user]
    if address:
        user = request.user
